...
public:
	...
	/*This function is called after a successful PlaceMove::execute function.
	ALGORITHM: Start at first tile place. First check upwards, keep going until beginning of "word" reached, 
	then go back in other direction, summing up points until end of "word" reached. Check in dictionary if it's
	a word; push word and score (ensuring correct mulitpliers are applied) into vector to be returned if it's a word, 
	fail function if not. Repeat for leftwards. 
	If either up or left (or both) immediately results in no letter (ie the tile is potentially a beginning letter
	for word), check downwards or rightwards respectively. After this first tile, the rest of the tiles only need to be 
	checked upwards (or downwards) if the word is horizontal, or leftwards (or rightwards) if the word is vertical.
	*/
	...
	*/
	/*This function is only called by the PlaceMove::execute function. This function first checks for usual errors: 
	that the tiles being placed don't go into squares already occupied, don't go out of bounds from the board,
	and have adjacent tiles already placed. It then uses the algorithm described above to ensure that all words
	being formed are legal. Returns true and places tiles if ok, false if any error was found.
	*/
	bool executePlaceMove (std::vector<Tile*> placeTiles, size_t x, size_t y, bool horizontal);
	...
	void placeTileOnBoard(Tile* currTile, size_t x, size_t y);
